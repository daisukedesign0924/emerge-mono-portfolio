<?php
/**
 * Plugin Name:       Emerge Mono
 * Plugin URI:        https://github.com/daisukedesign0924/emerge-mono
 * Description:       A monochrome, section-based site builder for creators. Compose your top page and pages by stacking sections in a live-preview Design Editor, with works gallery, profile/about, news, and a customizable contact form.
 * Version:           3.7.0
...
// ── MONO TOP（本体に統合：セクションビルダー型トップ＋ヒーロー＋Header/Footerタブ） ──
define( 'EMMT_VERSION', EMONO_VERSION );
define( 'EMMT_PATH', EMONO_PATH . 'includes/mono-top/' );
define( 'EMMT_URL',  EMONO_URL . 'includes/mono-top/' );
require_once EMMT_PATH . 'includes/mono-top.php';
require_once EMMT_PATH . 'includes/mono-top-sections.php';
require_once EMMT_PATH . 'includes/hero-section.php';
require_once EMMT_PATH . 'includes/header-footer.php';

// About ページのセクション編集（デザイン編集に About タブを追加）。
require_once EMONO_PATH . 'includes/about-scope.php';

// お問い合わせのフォームフィールド編集（デザイン編集に Contact タブを追加）。
require_once EMONO_PATH . 'includes/contact-scope.php';

// 投稿タイプの表示/非表示（管理画面）。
require_once EMONO_PATH . 'includes/post-type-visibility.php';

// MONO TOP の文字列は本体ドメイン（emerge-mono）に統合済み。
// 翻訳は本体の languages/emerge-mono-ja.mo が JIT で読み込む。

// MONO TOP フロント用アセット（本体スタイルの後）。
add_action( 'wp_enqueue_scripts', 'emono_monotop_front_assets', 20 );
function emono_monotop_front_assets() {
    wp_enqueue_style( 'emerge-mono-mono-top', EMMT_URL . 'assets/mono-top.css', array( 'emerge-mono' ), EMMT_VERSION );
    wp_enqueue_script( 'emerge-mono-mono-top', EMMT_URL . 'assets/mono-top.js', array(), EMMT_VERSION, true );
}

// デザイン編集でのセクション・ビルダー操作JS。
add_action( 'admin_enqueue_scripts', 'emono_monotop_admin_assets' );
function emono_monotop_admin_assets( $hook ) {
    if ( strpos( $hook, 'ene-top' ) === false ) {
        return;
    }
    wp_enqueue_script( 'emmt-builder', EMMT_URL . 'assets/builder.js', array( 'jquery-ui-sortable' ), EMMT_VERSION, true );
}
// ── エディター機能 ──
...
function emono_editor_menus() {
    add_menu_page( __( 'Design Editor', 'emerge-mono' ), __( 'Design Editor', 'emerge-mono' ), 'manage_options', 'ene-top', 'emono_design_editor_page', 'dashicons-art', 31 );
    add_menu_page( __( 'Post', 'emerge-mono' ),     __( 'Post', 'emerge-mono' ),     'edit_posts',  'ene-post',        'emono_ed_page_post',   'dashicons-edit-page',  32 );
    add_menu_page( __( 'All Works', 'emerge-mono' ),   __( 'All Works', 'emerge-mono' ),    'edit_posts',  'ene-works',       'emono_ed_page_works',  'dashicons-portfolio',  33 );
    add_menu_page( __( 'All News', 'emerge-mono' ), __( 'All News', 'emerge-mono' ), 'edit_posts',  'ene-news',        'emono_ed_page_news',   'dashicons-megaphone',  34 );
    add_menu_page( __( 'Page Manager', 'emerge-mono' ),   __( 'Page Manager', 'emerge-mono' ),   'edit_pages',  'ene-page-create', 'emono_ed_page_create', 'dashicons-admin-page', 35 );
    add_submenu_page( null, __( 'Edit Page', 'emerge-mono' ), __( 'Edit Page', 'emerge-mono' ), 'edit_pages', 'ene-page-edit', 'emono_ed_page_edit' );
}
...
function emono_enqueue_assets() {
    wp_enqueue_style( 'emerge-mono', EMONO_URL . 'assets/css/en-style.css', array(), EMONO_VERSION );
    ...
    if ( $inline_css !== '' ) {
        wp_add_inline_style( 'emerge-mono', $inline_css );
    }
    ...
    wp_enqueue_script( 'emerge-mono', EMONO_URL . 'assets/js/en-script.js', array(), EMONO_VERSION, true );
    wp_localize_script( 'emerge-mono', 'EN', array(
