...
add_shortcode( 'emerge_mono_terms', 'emono_shortcode_terms' );
function emono_shortcode_terms( $atts ) {
    ...
    $html = emono_terms_generate_html( $lang, $owner, $site, $email, $use_disclaimer, $custom );
    return '<div class="en-privacy">' . $html . '</div>';
}

// AJAX プレビュー
add_action( 'wp_ajax_en_preview_terms', 'emono_ajax_preview_terms' );
function emono_ajax_preview_terms() {
    ...
    $lang           = sanitize_key( (isset($_POST['terms_lang']) ? wp_unslash($_POST['terms_lang']) : 'ja') );
    ...
    $custom         = wp_kses_post( (isset($_POST['terms_custom']) ? wp_unslash($_POST['terms_custom']) : '') );

    wp_send_json_success( emono_terms_generate_html( $lang, $owner, $site, $email, $use_disclaimer, $custom ) );
}
